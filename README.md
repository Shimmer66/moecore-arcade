...
| 素材     | 自制占位图、60 张待审核 PNG、资源映射和素材清单           | 游戏引用与完整发布审核                    |
...
消消乐的 60 张 AI 生成 PNG 已纳入 `packages/assets/match3`，包含清单、来源记录及 SHA-256，状态为 `generated-pending-review`。当前应用尚未引用这些资源，素材入库不代表已完成公开游戏发布审核。许可私信及私密授权证据不提交到仓库。
