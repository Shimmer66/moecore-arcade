...
**当前阶段：双游戏可玩原型。** Vue 3 负责合集页面和游戏界面，纯 TypeScript 负责规则。
已接入一关 8×8 消消乐，以及带故事节点的 1,200 米星潮跑酷；不接入大模型 API，不需要模型账户，也不消耗真实 Token。
...
| 星潮跑酷 | 四角色、二段跳、滑铲、收集连击、冲刺、章节补给与多结尾      | 难度平衡、音效、真机验收             |
| 游戏 SDK | Vue 组件定义、统一 props / 事件、会话及结果类型             | 持久化服务接入                       |
| 角色     | 六位候选角色的稳定 ID、名称和待审核状态                     | 已获许可的角色头像和立绘             |
| 素材     | 60 张消消乐素材、400 张跑酷素材、延迟 URL 映射与校验        | 完整发布审核                         |
...
| AI 娘星潮跑酷                     | 已接入：把星光带回灯塔，沿途跳跃、滑铲和能量冲刺   |
...
- [星潮跑酷](docs/games/parkour.md)：故事、动作机制、操作和验收范围。
